...
public class Reaver : MonoBehaviour
{
    ...
    public void RunAI()
    {
        StartCoroutine(ReaverBrain());
    }

    IEnumerator ReaverBrain()
    {
        ...
        if (!uc.alreadyMoved)
        {
            ...
            if (paths[0].Count > 0)
            {
                Debug.Log("Reaver moving towards target!");
                uc.moving = true;
                StartCoroutine(ReaverMove(paths[0]));
                uc.alreadyMoved = true;
            }
            else
            {
                Debug.Log("Reaver didn't move towards target!");
            }
        }
        ...
        if (!uc.actionUsed)
        {
            ...
            if (targets.Count > 0)
            {
                Debug.Log("Reaver attacked target!");
                ReaverAttack(sm.GetUnit(targets[Random.Range(0, targets.Count - 1)].GetComponent<Tile>().position));
                yield return new WaitForSeconds(1f);
            }
            else
                Debug.Log("Reaver could not find a target!");
        }
    ...
    IEnumerator ReaverMove(List<Transform> tiles)
    {
    ...
    public void ReaverAttack(UnitController unit)
    {
        //get unit at pos
